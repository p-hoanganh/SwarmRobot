...
double max_vel_x=0.15;     //最大速度限制
double min_vel_x=0.02;     //最小速度限制
double max_vel_theta=2.0; //最大角速度限制
double min_vel_theta=0.4;//最小角速度限制
...
int main(int argc, char** argv){
  ros::init(argc, argv, "hawkbot_multi");

  ros::NodeHandle node;
  ros::NodeHandle private_nh("~");

  ros::Publisher slave_vel = node.advertise<geometry_msgs::Twist>("cmd_vel", 10); //发布原始数据
  ros::Subscriber vel_sub = node.subscribe("/robot_01/odom", 1, vel_Callback);

  private_nh.param<std::string>("base_frame", base_frame, "base_footprint");
  private_nh.param<std::string>("base_to_slave", base_to_slave, "robot_02");
  private_nh.param<double>("slave_x", slave_x,0.6);
  private_nh.param<double>("slave_y", slave_y,-0.6);
  private_nh.param<double>("max_vel_x", max_vel_x,0.15);
  private_nh.param<double>("min_vel_x", min_vel_x,0.02);
  private_nh.param<double>("max_vel_theta", max_vel_theta,2.0);
  private_nh.param<double>("min_vel_theta", min_vel_theta,0.4);
