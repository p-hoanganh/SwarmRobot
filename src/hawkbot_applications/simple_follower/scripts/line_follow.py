...
col_red = (0,136,38,203,255,255)# red
...
class Follower:
    ...
    def image_callback(self, msg):
        if self.temp == 0:
            ...
            cv2.createTrackbar(Switch,'Change_Color',0,4,nothing)
            self.temp = 1
        global last_erro
        ...
        m=cv2.getTrackbarPos(Switch,'Change_Color')
        if m == 0:
            lowerbH=col_red[0]
            lowerbS=col_red[1]
            lowerbV=col_red[2]
        ...
        if M['m00'] > 0:
            ...
            # 计算图像中心线和目标指示线中心的距离
                ...
                if erro!=0:
                    self.twist.angular.z = -float(erro)*0.004-float(d_erro)*0.000
                else :
